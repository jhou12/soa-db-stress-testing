...
const photos = async (productId) => {
  ...
  try {
    // const response = await fetchWithTimeout(`http://ec2-3-20-63-46.us-east-2.compute.amazonaws.com:4002/photos/id/${productId}`, { // use for requests to EC2!
    const response = await fetchWithTimeout(`http://localhost:4002/photos/id/${productId}`, { // use for requests to localhost!
      timeout: 3000
    });
...
const productInfo = async (productId) => {
  ...
  try {
    // const response = await fetchWithTimeout(`http://ec2-3-20-63-46.us-east-2.compute.amazonaws.com:4004/description/${productId}`, { // use for requests to EC2!
    const response = await fetchWithTimeout(`http://localhost:4004/description/${productId}`, { // use for requests to localhost!
      timeout: 3000
    });
